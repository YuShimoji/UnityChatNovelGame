...
**課題**:
- 現在の矛盾ペアはAI/人間の区別なく定義されている
- 「指摘対象 = AI のみ」の制約がコードに反映されていない
- HalluciCoin 以外の報酬（断片入手等）が矛盾発見に紐づいているが、概念的にはズレがある（後述）
...
**課題**:
- サブスレッド（A/B/C）との関係整理が必要
  - サブスレッドは「注釈/追跡/偵察強化」の3種固定
  - 分岐スレッドは「本筋からの If 分岐」で性質が異なる
- 分岐の発生条件・収束条件が未定義
