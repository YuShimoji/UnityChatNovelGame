...
### 方向性（未決定）
- データ型を分離するか統合したまま属性で区別するかは未定
- 断片インベントリ = 各キャラクターの所持品的なデータ閲覧要素でもある
- チャット上の無文脈な散文を「断片のようなもの」と呼ぶことはありえる（境界は曖昧）
...
1. **メカニクスの優先順位**: A/B/C のうちどれを最初に完成させるか
2. **断片の入手経路**: 偵察が未実装の現在、Yarn コマンド以外の入手手段はどうするか
3. **DeductionBoard の処遇**: 隔離して温存 / リファクタして活用 / 廃止して再設計
4. **断片インベントリの配置**: ダッシュボード内 / チャットからのスライド / 独立画面
5. **分岐スレッドとサブスレッドの関係**: 同じUIに統合 / 完全に別概念として分離
6. **矛盾発見 → 断片入手の紐づけ**: 外すか、別の形で残すか
