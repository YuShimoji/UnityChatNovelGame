...
3. **Pyramid 補足**（1対1）:
   ...
   - （締め）Hub「これからどうする」で**チームの次の動き**を語ったうえで、`Ch1_Day3_End` では **Pyramid 独白**として「端末外の観測が次の材料」「偵察拡大で照合が進む見込み」を短く言語化し、第2章（地名・現地確認の摩擦）への牽引力を補強する。
4. `<<SystemMessage "...">>` のあと `<<EndDay 3>>`（実装どおり）
