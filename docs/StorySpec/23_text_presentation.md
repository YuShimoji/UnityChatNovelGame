...
開発工程で仕様策定を進め、記載の確認時期にユーザー判断を求める。
...
| 項目 | 概要 | ステータス | 確認時期 |
|------|------|-----------|---------|
| スレッド別背景 | スレッドごとにチャット背景画像/色を変更 (壁紙)。ThreadSwitcherController.SwitchToThread 時に背景を切替 | 保留: サブクエスト本格実装時に判断 | サブクエスト本格実装時 |
| インライン画像 | バブル内テキストに画像を混在させる。TMP Sprite Asset を利用 | 保留: C 型成果物カード着手時に TMP Sprite Asset の採用を判断 | 成果物カード (C 型) 着手時 |
| メッセージグルーピング | 日付区切り (SP-024 S1)、連続メッセージの視覚的グループ化 (間隔圧縮 + 角丸連結) | 保留: SP-024 S1 (タイムスタンプ) 実装後に統合判断 | タイムスタンプ実装後 |
| 引用/返信 | 「このメッセージに返信」表現。元メッセージの引用バーをバブル上部に表示 | 保留: チャットアプリ没入度の優先度判断 | SP-024 実装後 |
