# LATER: Ch2 執筆プレイブック（SUBSEQUENT ゲート通過後・Ch1 フルコンテンツ完了後）

[docs/project-context.md](../project-context.md) の **LATER RECOMMENDED SLICE** を実行する際の短い手順書。本文は仕様の正典ではなく **オペレーション用**。

## 前提

- エンジン能力マイルストーン M1-M2 完了 + SUBSEQUENT ゲート通過済み: P0 が 0 件。
- Ch1 フルコンテンツ前進が完了し、Ch1 で検証した全エンジン能力の PlayMode テストカバレッジが確認済み。
...
5. **BL-002（ポートレート）**: Ch2 の視認性がボトルネックと判断した時だけ着手（M7）。
