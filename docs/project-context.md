...
- **Phase 2–4**（Alpha ゲート、ENH 集中、製品化）は `docs/FEATURE_REGISTRY.md`・CLAUDE.md のロードマップと整合。詳細テーブルが必要なら履歴コミット参照。
...
## HANDOFF SNAPSHOT (2026-04-10、技術状態は session 22 まで反映)

- 主レーン: **Content**（Ch1 前進） / 副: **Unlock**（Pipeline 実運用）
- **CURRENT スライス**: Ch1 コンテンツ前進 + パイプライン実運用。Ch1 既知 UI は **UI_ISSUES.md**（個別コード修正はバッチまで）
- **session 22 で完了した技術作業**: タイプライター同期（DOTween 完了待機）、DebugChatScene 整備（用途に応じた StartNode・ChatDialogueView 等）、SaveManager AutoSaveIndicator の null 安全化、PlayMode **8**/8・EditMode 75/75（ローカル）、SafeTeardown・batch XML
- **次に推奨（NEXT）**: **SP-022**（`docs/StorySpec/22_subquest_exploration_content.md`）を埋め、Ch1 にサブクエスト パイロット **1〜3 本**
- **その次（SUBSEQUENT）**: Ch1 統合プレイ + ギャップ **P0/P1/P2**。**HANDOFF.md 手動ハンズオン** → SP-017 Ch1 例 → 好機に EN-012 → **Ch2 執筆（S21–22）**
- **その次（LATER）**: Ch2 本編＋サブ。**P0 のみ**短い仕様／実装スライス可（詳細は本文「推奨プランの読み方」、判断は HANDOFF チェックリスト）
- 次にやること（CURRENT）:
  1. Ch1 を Yarn で前に進める（Day／節）
  2. Content Pipeline → Sync Authoring Assets → 本編で再生確認
  3. **StartNode** はシナリオ目的に合わせて Inspector で確認
- 好機に: DQT/Ch1/Ch2 通し、**GitHub Actions CI**、PlayMode 実ラン記録（verification/）
- 既知の軽微な問題: DQT 選択肢後スキップ、連打時メッセージ遷移（ENH 候補）
- 設計論点: IP-PC-002（スレッド管理）、IP-PC-001（ポートレート・HUMAN_AUTHORITY）
- 今は触らない: UI_ISSUES の個別修正、サウンド、マネタイズ、**B 型 Wiki のエンジン新規実装**（SP-022 ギャップ扱い）
