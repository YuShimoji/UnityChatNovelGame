...
- まだ未実施（好機に）: PlayMode 4 件の実ラン記録更新、E2E の継続拡張（EN-012）
...
- **補足**: `DebugChatScene` の `m_StartNode` は用途により異なる（本編・VerticalSlice・DQT など）。**再生目的に合わせて** Inspector で確認すること。session 21 当時の「Start → DQT_Start」指示は、当時の欠落ノード対策文脈向け
...
4. **デバッグ小出し**: エンジン挙動だけ試すときは `DebugChatScene` を開き、**目的に応じて** `m_StartNode` を `DQT_Start` や `VerticalSlice_Start` 等に設定（欠落ノードを指さないこと）
5. **好機にのみ**: Unity Test Runner で PlayMode テスト 4 件を実行し、結果を `docs/verification/` 等へ残す（EN-012）
...
10. **（同上）** `docs/StorySpec/17_unlock_triggers.md` に Ch1 用具体例を 1 ページ追記。好機に PlayMode 4 件の結果を `docs/verification/` に 1 ファイル保存。その後 **Ch2 執筆**へ移行
