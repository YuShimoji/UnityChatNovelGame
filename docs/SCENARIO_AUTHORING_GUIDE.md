...
| ~~MessageTagged~~ | `<<MessageTagged "charID" "テキスト" "lineTag">>` | 削除済み。`#line:` タグ方式を使うこと |
...
```
// エラー: 同じ #line: タグが複数ファイルに存在
Error: program_invalid (Yarn プロジェクト全体のコンパイル失敗)
```

`#line:` タグは **Yarn プロジェクト全体で一意** でなければならない。
異なるファイルで同じタグ名を使うとプロジェクト全体がコンパイルエラーになり、全シナリオが動作しなくなる。

テスト用に矛盾ペアを複製する場合は、必ず独自のタグ名を付けること:

```yarn
// 間違い: Ch1 と同じタグ名をテスト用に再利用
対象地域は... #line:ch1_region_identity_src

// 正しい: テスト用の独自タグ名
対象地域は... #line:etk_region_identity_src
```
