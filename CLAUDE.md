...
**ガードレールと実行計画の関係:** 本節の DEVELOPMENT PURPOSE とスコープ境界は、実行計画（`docs/project-context.md` の CURRENT DEVELOPMENT AXIS 等）より**上位**の制約である。実行計画がガードレールと矛盾する場合、ガードレールが優先される。具体的には、CURRENT AXIS がコンテンツ前進を主軸とする場合でも、「エンジン/ツール能力の前進なしにコンテンツのみ積む」状態が 2 セッション以上続いたら、実行計画を見直す。
...
- コンテンツ量の目安: 1機能につき1-2ノードの検証モック。フルチャプター執筆はエンジン能力マイルストーン（project-context.md の ENGINE CAPABILITY MILESTONES）の各ゲートを通過した後に、そのゲートで検証された範囲内でのみ進める
